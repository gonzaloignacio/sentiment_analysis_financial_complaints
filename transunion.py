...
Original file is located at
    https://colab.research.google.com/drive/1FfSios4IjN9cd3q0O2xuHooLHIWqxQMP
"""
...
df = pd.read_excel('/content/drive/MyDrive/Colab Notebooks/Social Media Analytics/TRANSUNION INTERMEDIATE HOLDINGS INC..xlsx')
...
sns.histplot(data = df, x = "pos-neg score", color = "skyblue")
plt.title("Distribution of scores for Transunion")
...
sns.countplot(data = df, x = "sentiment class", color = "skyblue")
plt.title("Distribution of sentiment classes for Transunion")
...
plt.imshow(wordcloud, interpolation="bilinear")
plt.axis("off")
plt.title("Word Cloud of Most Common Words in Complaints with negative sentiment for Transunion")
plt.show()
...
sns.barplot(x = list(counts_und), y = list(words_und))
plt.title("Most common words in complaints with undetected sentiment")
...
sns.barplot(x = list(counts_neg), y = list(words_neg), color = "skyblue")
plt.title("Most common words in complaints with negative sentiment for Transunion")
plt.xlabel("Frequency")
plt.ylabel("Word")
#plt.xticks(rotation = 45)
plt.show

df.groupby("sentiment class").size()